...
#library(remotes)
#remotes::install_github("ices-tools-dev/icesRDBES", ref = "dev")
library(icesRDBES)
library(DT)
library(dplyr)
library(plotly)
...
## Tree Diagram for standard deviation of Vessel Selection Strata

Plot showing the standard deviation for the vessel selection strata. The tree displays the parent strata, Design and Sampling Details. Click on a node to expand its child strata. 
...
All_Data <- full_join(VS, 
  ...
  mutate(var.sd = 
           paste0(stratumName,': VS stand. deviat. - ',
                  format(round(as.numeric(var.sd), 0), big.mark=","))) %>%
...
DE <- DE %>%
  select(recType, stratumName, DE.Total, var.sd, Year, stratumName)
# # plot
...
SD <- SD %>%
  select(recType, stratumName, SD.Total, var.sd, parentTableStratum) %>%
...
Graph 2 displays the standard deviation by Vessel Selection stratum name coloured by Design stratum name.

Graph 3 displays the standard deviation by Vessel Selection stratum name coloured by the Sampling Details stratum name.
...
# Standard deviatione for VS by VS stratum name coloured by DE Stratum Name
VS_scatter1 <-ggplot(data=VS1,
                      aes(x=stratumName, y=var.sd, fill = stratumName.y)) +
  geom_point(color="cadetblue", alpha= 0.7, size=4) +
  labs(title="VS Stand. Deviat. by VS Stratum Name",
       x="VS Stratum Name",
...
# Standard deviation for VS by VS stratum name coloured by SD Stratum Name
VS_scatter2 <-ggplot(data=VS1,
                     aes(x=stratumName, y=var.sd, fill = stratumName.x)) +
  geom_point(color="skyblue4", alpha= 0.7, size=4) +
  scale_fill_tech(theme="airbnb") +
  labs(title="VS Stand. Deviat. by SD Stratum Name",
       x="VS Stratum Name",
       y = "Standard deviation") +
  theme_tech(theme="airbnb") +
  theme(plot.title = element_text(color = "grey30")) +
...
# tidy df for table
VS2 <- VS %>%  
  select(recType, stratumName, parentTableStratum, est.total, var.sd)
...
Graph 2 displays the standard deviation by Vessel Selection stratum name.
...
FO_scatter1 <-ggplot(data=FO, aes(x=VS.stratumName, y=var.sd.x))+
  geom_point(color="aquamarine4", alpha= 0.7, size = 4) +
  labs(title="FO Stand. Deviat. by VS Stratum",
       x="VS Stratum Name",
       y = "Standard deviation") +
 theme_tech(theme="airbnb") +
 theme(plot.title = element_text(color = "grey30")) +
...
3D graph showing the standard deviation and estimated total per Year, coloured by Sampling Details stratum name.
...
p <- p %>% add_markers()
p <- p %>% 
  layout(title = paste0('Stand. Deviat. and Estimated Total for',
                          'Vessel Selection per Year by SD Stratum'),
         scene = list(xaxis = list(tickvals= ~Year.y, title = 'Year'),
         yaxis = list(title = 'Stand. Deviat. VS'),
