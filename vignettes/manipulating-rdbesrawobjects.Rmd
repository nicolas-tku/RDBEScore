...
myH1RawObject <-
     createRDBESRawObject(rdbesExtractPath = "..\\tests\\testthat\\h1_v_1_19_13")
...
myH5RawObject <-
     createRDBESRawObject(rdbesExtractPath = "..\\tests\\testthat\\h5_v_1_19_13")
...
RDBESRawObjects can be combined using the combineRDBESRawObjects() function.  This might be required when different sampling schemes are used to collect on-shore and at-sea samples - it will often be required to combine all the data together before further analysis.
...
validateRDBESRawObject(myFilteredObject, verbose = FALSE)
```
...
validateRDBESRawObject(myFilteredObjectValidVesselLinks, verbose = FALSE)
...
myFilteredObjectValidSpeciesLinks <- filterRDBESRawObject(myFilteredObjectValidVesselLinks,
                                         fieldsToFilter = myFields,
                                         valuesToFilter = myValues )
...
validateRDBESRawObject(myFilteredObjectValidSpeciesLinks, verbose = FALSE)
