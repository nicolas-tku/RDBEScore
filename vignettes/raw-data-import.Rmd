...
It should be noted that the objects created are of the S3 class "RDBESRawObject"
...
RDBESRawObject structure can be validated using the validateRDBESRawObject() function.

```{r}
importedTables <- importRDBESDownloadData(zipFiles)
validateRDBESRawObject(importedTables, verbose = FALSE)
```
