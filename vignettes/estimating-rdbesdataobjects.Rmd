...
vignette: >
  %\VignetteIndexEntry{Estimating RDBESDataObjects}
...
```{r load}
h1d <- createRDBESDataObject(rdbesExtractPath = "..\\tests\\testthat\\h1_v_1_19_13")
h1dValid <- validateRDBESDataObject(h1d, verbose = FALSE)
...
```
The expected total and its variance for **unbiased estimation** are 36469.4 and 3375.61. 
The expected mean and its variance for **ratio estimation** are 5.48 and 0.029. 
For **ratio estimation**  the expected total and its variance are to be the same as for unbiased estimation. See below:
...
estM <- 555/10*120
estM
sbRes$estim$est.mean * estM
sbRes$estim$est.total / M
...
```{r}
estimTables <- icesRDBES::getTablesInRDBESHierarchy(1)
estimTables <- estimTables[3:9]
estimTables
```   
```{r}
...
```{r}
startEstim <- list(parentIdCol = "VSid", ids=c(63301))
