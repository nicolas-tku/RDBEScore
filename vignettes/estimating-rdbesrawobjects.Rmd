...
estimate<-function(tbl, estimRes, target){
  ...
  if(length(missingCols) == 0){
    ...
    if(!is.null(estimRes$prevStage)){
      ...
      estim$est.total <- lastEstim$tHat * estim$est.total
      estim$est.mean <- lastEstim$tHat  * estim$est.mean 
    }
...
set.seed(1234)
hd<-makeTbls(LETTERS[1:5], method="SRSWOR", proportionSampled = 0.5)
```
...
aggLevelEstimations <- function(estims){
  ...
  tHat = mean(sapply(estims, function(x){mean(1/x$PI/x$Nn/x$Nn )})),
  Nn = mean(sapply(estims, function(x){mean(x$Nn)}))
  ))
}
```
