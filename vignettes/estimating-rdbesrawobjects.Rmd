...
aggLevelEstimations <- function(estims){
  if(length(estims)<2){
    estims[[1]]$tHat.total <- sum(1/estims[[1]]$PI)
    estims[[1]]$tHat.mean <- sum(1/estims[[1]]$PI)
    return(estims[[1]])
  }
  return(list(
  ...
  PI = sum(sapply(estims, function(x){x$PI})),
  tHat.total = sum(sapply(estims, function(x){sum(1/x$PI)})),
  tHat.mean = mean(sapply(estims, function(x){sum(1/x$PI)}))
  ))
}
```
...
estimate<-function(tbl, estimRes, target){
  ...
  if(length(missingCols) == 0){
    ...
    if(!is.null(estimRes$prevStage)){
      ...
      estim$est.mean <- lastEstim$tHat.mean  * estim$est.mean / nrow(data)
    }
...
makeTbl <- function(tblName, prevTbls = list(), rows=4,
  ...
  if(!is.null(prevTbl)){
    ...
    cols <- c(cols, grep("id$", colnames(prevTbl), value = TRUE))
  }
  data <- list(1:times,
               rep(rows, times),
               rep(rows, times),
