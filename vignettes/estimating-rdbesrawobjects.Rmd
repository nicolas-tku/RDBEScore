...
estimate<-function(tbl, estimRes, target){
  ...
  if(length(missingCols) == 0){
    ...
    if(!is.null(estimRes$prevStage)){
      ...
      estim$est.total <- lastEstim$tHat.total * estim$est.total
      estim$est.mean <- lastEstim$tHat.mean  * estim$est.mean
    }
...
```{r}
check<-function(x,e, digits=2, show=F){
  if(show){
    if(x != e){
...
```{r}
makeTbl <- function(tblName, prevTbls = list(), rows=4,
...
aggLevelEstimations <- function(estims){
  if(length(estims)<2){
    estims[[1]]$tHat.total <- mean(1/estims[[1]]$PI)
    estims[[1]]$tHat.mean <- mean(1/estims[[1]]$PI)
    estims[[1]]$PI <- mean(1/estims[[1]]$PI)
    return(estims[[1]])
  }
  return(list(
  ...
  tHat.total = mean(sapply(estims, function(x){mean(1/x$PI)})),
  tHat.mean = mean(sapply(estims, function(x){mean(1/x$PI)}))
  ))
}
```
...
check(res$estim$est.total, sum(b$By))
...
check(res$estim$var.mean, 0)
check(res$estim$est.total, sum(b$Cy))
check(res$estim$var.total, 0)
nrow(b)
```
...
check(res$estim$var.mean, 0)
check(res$estim$est.total, sum(b$Dy))
check(res$estim$var.total, 0)
nrow(b)
```
...
check(res$estim$var.mean, 0)
check(res$estim$est.total, sum(b[[Ty]]))
check(res$estim$var.total, 0)
nrow(b)
```
